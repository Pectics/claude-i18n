...
test('renders malformed target JSON as invalid without replacing a valid language count', () => {
  ...
  try {
    ...
    writeJson(path.join(fixture.targetRoot, 'zh-Hans', 'zh-Hans.dynamic.json'), {});
    fs.mkdirSync(path.join(fixture.targetRoot, 'zh-Hant'), { recursive: true });
    fs.writeFileSync(path.join(fixture.targetRoot, 'zh-Hant', 'zh-Hant.json'), '{invalid', 'utf8');
    writeJson(path.join(fixture.targetRoot, 'zh-Hant', 'zh-Hant.dynamic.json'), {});
    const payload = runGenerate(fixture);
    assert.deepEqual(payload['zh-Hans'], { covered: 1, total: 2, ratio: 0.5 });
    assert.deepEqual(payload['zh-Hant'], { covered: null, total: 2, ratio: null });
  } finally {
    fs.rmSync(fixture.root, { recursive: true, force: true });
  }
});
